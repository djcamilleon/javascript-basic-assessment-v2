...
function secret(obj){
  for(var prop in obj){
    if(prop.startsWith('secret') === true){
      obj[prop] = 'redacted'
    }
  }
  return obj;
...
// var jedi = ['Mario', 'Mace Windu','Luke'];
...
// The order of the array should not be changedprop
...
function tripleTheFun(thing){
  if(typeof(thing) === 'string' && Number(thing)){
...
function whatIsItOutside(temp, humidity,clouds){
  ...
  else if(temp > 85 && humidity < 45 && clouds < 20){
    return "I'm literally in the desert"
  }
  else if(temp > 90 || humidity > 55 || clouds > 65){
